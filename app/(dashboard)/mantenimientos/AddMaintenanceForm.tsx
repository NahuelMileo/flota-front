  ...
  const onSubmit = async (data: MaintenanceFormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.message || error.title || JSON.stringify(error) || "Error al crear mantenimiento");
      }
      ...
      const message =
        error instanceof Error ? error.message : "Error al crear mantenimiento";
      toast.error(message);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Button type="submit" disabled={isLoading} className="w-full">
        {isLoading ? "Guardando..." : "Registrar mantenimiento"}
      </Button>
    </form>
  );
}
